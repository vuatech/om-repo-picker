...
const repo repos[] = {
	...
	{ "extra", QT_TRANSLATE_NOOP("repos", "Extra"), QT_TRANSLATE_NOOP("repos", "Extra contains packages that are free, but not officially supported by OpenMandriva. While we will try to fix them if something breaks, this is a low priority. Enable Extra if you need them and don't mind an occasional crash."), nullptr, nullptr },
...
bool disableRepos(QStringList const &repos) {
	...
	args << "config-manager" << "setopt" << repos;
